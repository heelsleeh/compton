...
static void
compose(void *backend_data, session_t *ps,
  win *w, void *win_data,
  int dst_x, int dst_y,
  const region_t *reg_paint)
{
  ...
  if (w->shadow) {
    ...
    if (pixman_region32_not_empty(&reg_tmp)) {
      ...
      xcb_render_composite(ps->c, XCB_RENDER_PICT_OP_OVER, wd->shadow_pict, alpha_pict,
        xd->target_buffer, 0, 0, 0, 0, dst_x + w->shadow_dx, dst_y + w->shadow_dy,
        w->shadow_width, w->shadow_height);
    }
  ...
  xcb_render_composite(ps->c, op, wd->rendered_pict, alpha_pict,
    xd->target_buffer, 0, 0, 0, 0, dst_x, dst_y, w->widthb, w->heightb);
}
...
static bool
blur(void *backend_data, session_t *ps, double opacity,
  const region_t *reg_paint)
{
  struct _xrender_data *xd = backend_data;
  ...
  // The multipass blur implemented here is not correct, but this is what old compton
  // did anyway. XXX
  ...
  for (i = 0; ps->o.blur_kerns[i]; i++) {
    ...
    if (ps->o.blur_kerns[i+1] || i == 0) {
      ...
      xcb_render_composite(ps->c, XCB_RENDER_PICT_OP_OVER, src_pict, alpha_pict,
        xd->target_buffer, 0, 0, 0, 0, reg->x1, reg->y1, width, height);
    }
...
static void
render_win(void *backend_data, session_t *ps,
  win *w, void *win_data, const region_t *reg_paint)
{
  ...
  if (wd->buffer == XCB_NONE) {
    wd->buffer = x_create_picture_with_pictfmt(ps, w->widthb, w->heightb, w->pictfmt, 0, NULL);
  }
  ...
  if (w->invert_color) {
    ...
    xcb_render_composite(ps->c, XCB_RENDER_PICT_OP_DIFFERENCE, xd->white_pixel, None,
      wd->rendered_pict, 0, 0, 0, 0, 0, 0, w->widthb, w->heightb);
    // We use an extra PictOpInReverse operation to get correct pixel
    // alpha. There could be a better solution.
    if (win_has_alpha(w))
      xcb_render_composite(ps->c, XCB_RENDER_PICT_OP_IN_REVERSE, wd->pict, None,
          wd->rendered_pict, 0, 0, 0, 0, 0, 0, w->widthb, w->heightb);
  }
  ...
  if (w->frame_opacity != 1) {
    ...
    xcb_render_picture_t alpha_pict = xd->alpha_pict[(int)(w->frame_opacity * dopacity * 255)];
  ...
  if (w->dim) {
    ...
    x_clear_picture_clip_region(ps, wd->rendered_pict);
    xcb_render_fill_rectangles(ps->c, XCB_RENDER_PICT_OP_OVER, wd->rendered_pict,
      color, 1, &rect);
  }
}

static void*
prepare_win(void *backend_data, session_t *ps, win *w) {
  ...
  printf_errf("(): %s %x", w->name, wd->pixmap);
  wd->pict = x_create_picture_with_pictfmt_and_pixmap(ps, w->pictfmt,
    draw, 0, NULL);
  wd->buffer = XCB_NONE;
  ...
  if (w->shadow) {
    xcb_pixmap_t pixmap;
    build_shadow(ps, 1, w->widthb, w->heightb, xd->shadow_pixel,
      &pixmap, &wd->shadow_pict);
...
static void
release_win(void *backend_data, session_t *ps, win *w, void *win_data) {
  struct _xrender_win_data *wd = win_data;
...
static void *init(session_t *ps) {
  ...
  if (ps->overlay != XCB_NONE) {
    xd->target = x_create_picture_with_visual_and_pixmap(ps,
      ps->vis, ps->overlay, 0, NULL);
  } else {
  ...
  xd->target_buffer = x_create_picture_with_visual(ps, ps->root_width,
      ps->root_height, ps->vis, 0, NULL);
  ...
  if (root_pixmap == XCB_NONE) {
    ...
    xd->root_pict = x_create_picture_with_visual_and_pixmap(ps, ps->vis,
      root_pixmap, 0, NULL);
  }
  return xd;
}
...
static void paint_root(void *backend_data, session_t *ps,
  const region_t *reg_paint)
{
  struct _xrender_data *xd = backend_data;
  ...
  xcb_render_composite(ps->c, XCB_RENDER_PICT_OP_SRC, xd->root_pict, XCB_NONE,
    xd->target_buffer, 0, 0, 0, 0, 0, 0, ps->root_width, ps->root_height);
}
...
static void present(void *backend_data, session_t *ps) {
  ...
  xcb_render_composite(
    ps->c, XCB_RENDER_PICT_OP_SRC, xd->target_buffer, None,
    xd->target, 0, 0, 0, 0,
